...
public class UpperPrice extends TechAnalyzer {
	private static Logger log = LoggerFactory.getLogger(UpperPrice.class);
	
	public UpperPrice(TechAnalyzerManager techAnalyzerManager) {
		super(techAnalyzerManager);
	}
	
	@Override
	public String getCacheName(String product) {
		return StringCombineUtil.combine(UpperPrice.class.getSimpleName(), product);
	}
	...
	public List<AnalysisResult> analyze(DataSource dataSource, String product, Date from, Date to) throws Exception {
		...
		try {
			...
			for(ProductClosingInfo prodInfo : lstProductInfo) {
				...
				result.setValue(prodInfo.getUpperPrice());
