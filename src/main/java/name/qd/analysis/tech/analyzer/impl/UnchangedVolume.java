...
public class UnchangedVolume implements TechAnalyzer {
		...
		try {
			...
			for(Date date : map.keySet()) {
				...
				for(ProductClosingInfo info : map.get(date)) {
					if(info.getADStatus() == ProductClosingInfo.UNCHANGE) {
			...
			log.error("UnchangedVolume analyze failed.", e);
			throw e;
		}
		return lstResult;
	}
	...
	public List<AnalysisResult> customResult(DataSource dataManager, String product, Date from, Date to, String... inputs) throws Exception {
		...
		List<AnalysisResult> lst = TechAnalyzerManager.getInstance().getAnalysisResult(dataManager, Analyzer.UnchangedVolume, product, from, to);
