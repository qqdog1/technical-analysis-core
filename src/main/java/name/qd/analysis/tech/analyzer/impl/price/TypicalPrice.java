...
public class TypicalPrice extends TechAnalyzer {
	private static Logger log = LoggerFactory.getLogger(TypicalPrice.class);

	public TypicalPrice(TechAnalyzerManager techAnalyzerManager) {
		super(techAnalyzerManager);
	}
	
	@Override
	public String getCacheName(String product) {
		return StringCombineUtil.combine(TypicalPrice.class.getSimpleName(), product);
	}
	...
	public List<AnalysisResult> analyze(DataSource dataSource, String product, Date from, Date to) throws Exception {
		...
		try {
			...
			for(ProductClosingInfo info : lstProductInfo) {
				...
				double value = info.getUpperPrice() + info.getLowerPrice() + info.getClosePrice();
				result.setValue(value / 3);
