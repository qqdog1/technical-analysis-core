...
public class TWSEDataPoller2016 extends DataPoller {
	...
	public TWSEDataPoller2016(DataPoller nextPoller) {
		super(nextPoller);
	}
	...
	public void tryDownloadProdClosingInfo(String year, String month, String prodId) throws IOException {
		...
		FileOutputStream fos = new FileOutputStream(FileConstUtil.getProdClosingFilePath(year, month, prodId));
	...
	public void tryDownloadDailyClosingInfo(String date) throws IOException {
		...
		ReadableByteChannel rbc = Channels.newChannel(connection.getInputStream());
		@SuppressWarnings("resource")
		FileOutputStream fos = new FileOutputStream(FileConstUtil.getDailyClosingFilePath(date));
		fos.getChannel().transferFrom(rbc, 0, Long.MAX_VALUE);
