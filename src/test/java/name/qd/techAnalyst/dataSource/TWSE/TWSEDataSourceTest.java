	...
	public void testDataParser() {
		...
		try {
			...
			for(ProductClosingInfo p : lst) {
				String s = StringCombineUtil.combine(sdf.format(p.getDate()), ":", String.valueOf(p.getFilledShare()), ",", String.valueOf(p.getFilledAmount()), ",", 
						String.valueOf(p.getOpenPrice()), "," ,
						String.valueOf(p.getUpperPrice()), "," ,
						String.valueOf(p.getLowerPrice()), "," ,
