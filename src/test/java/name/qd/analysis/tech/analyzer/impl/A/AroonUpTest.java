...
public class AroonUpTest {
	...
	public void customTest() {
		try {
			List<AnalysisResult> lst = manager.getCustomAnalysisResult(dataSource, TechAnalyzers.Aroon_UP, "1101", from, to, days);
			...
			for (int i = 0; i < lst.size(); i++) {
				...
				for(int x = 0; x <= iDays; x++) {
					double price = lstProducts.get(i+x).getUpperPrice();
					if(price > max) {
						max = price;
						index = x;
					}
